...
const M3U_DEFAULT = "https://iptv-org.github.io/iptv/index.m3u";
...
function getStyle(cat) {
  for (const key of Object.keys(catStyle)) {
    if (cat?.toLowerCase().includes(key.toLowerCase())) return catStyle[key];
  }
  return catStyle.Other;
}
...
function parseM3U(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("#EXTINF")) {
      ...
      channels.push({ ...meta, url: line, uid: `${meta.name}-${channels.length}` });
      meta = null;
    }
  }
  return channels;
...
function HLSPlayer({ url, onError }) {
    ...
    const load = async () => {
      try {
        if (video.canPlayType("application/vnd.apple.mpegurl")) {
          ...
          const Hls = (await import("https://cdn.jsdelivr.net/npm/hls.js@1.5.7/dist/hls.esm.min.js")).default;
          if (!Hls || !Hls.isSupported()) { onError?.(); return; }
  ...
  return (
    <video ref={videoRef} controls autoPlay playsInline
      style={{ width: "100%", display: "block", background: "#000", maxHeight: 230 }} />
  );
}
      ...
      <div style={{ padding: "12px 12px 14px", display: "flex", flexDirection: "column", gap: 9, flex: 1 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <button onClick={() => onToggleFav(ch.uid)}
            style={{ background: "none", border: "none", cursor: "pointer", color: isFav ? "#ec4899" : dark ? "#374151" : "#d1d5db", padding: 0 }}>
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ background: "#000" }}>
          {err
            ? <div style={{ height: 200, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, color: "#4b5568" }}>
                <span style={{ fontSize: 40 }}>📡</span>
                <span style={{ fontWeight: 800, fontSize: 14, color: dark ? "#94a3b8" : "#64748b" }}>Stream unavailable</span>
                <span style={{ fontSize: 11, color: dark ? "#4b5568" : "#94a3b8", textAlign: "center", maxWidth: 220 }}>May be geo-restricted or offline</span>
              </div>
        ...
        <div style={{ padding: "18px 18px 28px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            ...
            <button onClick={onClose} style={{ background: dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)", border: "none", borderRadius: 11, padding: 8, cursor: "pointer", color: dark ? "#94a3b8" : "#64748b", display: "flex" }}>
              <IcoX />
            </button>
          </div>
...
function SourcesTab({ dark, onLoadChannels }) {
  const [primary, setPrimary] = useState(M3U_DEFAULT);
  ...
  const handleLoad = async () => {
    ...
    for (const url of urls) {
      try {
        ...
        setMsg(`⚠️ ${url === primary ? "Primary" : "Fallback"} failed…`);
      }
    }
    setMsg("❌ All sources failed.");
    setLoading(false);
  };

  const card = { background: dark ? "rgba(255,255,255,0.04)" : "#fff", border: dark ? "1px solid rgba(255,255,255,0.07)" : "1px solid rgba(0,0,0,0.07)", borderRadius: 18, padding: "18px", marginBottom: 14 };

  return (
    <div style={{ padding: "18px 16px 100px" }}>
      <div style={card}>
        {label("Primary M3U Source")}
        <input style={inp} value={primary} onChange={e => setPrimary(e.target.value)} placeholder="https://iptv-org.github.io/iptv/index.m3u" />
      </div>

      <div style={card}>
        {label("Fallback URL (optional)")}
        <input style={{ ...inp, marginBottom: 14 }} value={fallback} onChange={e => setFallback(e.target.value)} placeholder="https://backup.com/playlist.m3u" />
      ...
      {msg && (
        <div style={{ padding: "12px 14px", borderRadius: 13, background: dark ? "rgba(255,255,255,0.04)" : "#f8fafc", border: dark ? "1px solid rgba(255,255,255,0.06)" : "1px solid rgba(0,0,0,0.06)", fontSize: 12, color: dark ? "#94a3b8" : "#64748b", marginBottom: 14 }}>
          {msg}
        </div>
      )}
...
export default function NileTV() {
  ...
  const toggleFav = uid => setFavorites(p => p.includes(uid) ? p.filter(f => f !== uid) : [...p, uid]);

  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(CORS + encodeURIComponent(M3U_DEFAULT));
        const text = await res.text();
  ...
  return (
    <div style={{ background: bg, minHeight: "100vh", fontFamily: "'Plus Jakarta Sans',sans-serif", color: tx }}>
      ...
      {/* Header */}
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: dark ? "rgba(13,15,26,0.92)" : "rgba(240,242,247,0.92)", backdropFilter: "blur(14px)", borderBottom: bd, padding: "10px 16px", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ flexShrink: 0 }}>
          ...
          <div style={{ fontSize: 9, color: sub, fontWeight: 600 }}>{loading ? "Loading…" : `${channels.length} channels · Free live TV`}</div>
        </div>
        {tab === "channels" && (
        ...
        {tab !== "channels" && <div style={{ flex: 1 }} />}
        <button onClick={() => setDark(d => !d)} style={{ background: dark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)", border: "none", borderRadius: 11, padding: 8, cursor: "pointer", color: sub, display: "flex", flexShrink: 0 }}>
          {dark ? <IcoSun /> : <IcoMoon />}
        </button>
      </div>
      ...
      {tab === "channels" && (
        <>
          ...
          <div style={{ margin: "14px 14px 0", padding: "20px 18px", borderRadius: 18, background: "linear-gradient(135deg,rgba(124,58,237,0.18),rgba(236,72,153,0.12))", border: dark ? "1px solid rgba(255,255,255,0.06)" : "1px solid rgba(124,58,237,0.12)" }}>
            ...
            <div style={{ fontSize: 11, color: sub }}>{channels.length} curated · <span style={{ color: "#10b981", fontWeight: 700 }}>{channels.length.toLocaleString()} in index</span></div>
        ...
        {[
          ...
          return (
            <button key={key} onClick={() => setTab(key)} style={{
              ...
              color: active ? "#7c3aed" : sub, fontFamily: "inherit", padding: "0 20px"
            }}>
